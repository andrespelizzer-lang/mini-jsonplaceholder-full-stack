<template>
  <div>
    ...
    <template v-else>
      <p v-if="utentiFiltrati.length === 0" class="vuoto">Nessun utente trovato</p>
      <CardUtente
...
const { ottieniUtenti, creaUtente, aggiornaUtente, eliminaUtente: eliminaUtenteApi } = useUtenti()
...
async function salvaUtente(dati: Record<string, string>) {
  ...
  try {
    ...
    errore.value = err instanceof Error ? err.message : 'Errore nel salvataggio'
  }
}
